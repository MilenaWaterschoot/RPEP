...
input <- data.frame()
files <- dir(pattern = "RPEP_data_") #files; double _ so that it detects underscore
...
input$startPrev <- lag(input$startRT) # Start RT of trial n-1
## and make it numeric
...
```{r}
# check overall if design worked
#table(input$participant, input$outcome) # this shows for each participant how many -5 outcomes there were and how many +5 (and how many undefined (i.e., 2))
#table(input$participant, input$choiceRT) # I don't understand this table
...
```{r}
# STEP 4: exclusion
# this subject might need to be excluded: is missing a trial
subject_excl <- c(66)
...
# do I also need to remove the first trial of the random sequence? i.e., 19, 49, 79, 109
...
save(input, file = "C:\\Users\\milen\\Dropbox\\Universiteit\\MASTER\\RPEP\\RPEP data analysis\\dataframe input.Rdata")
```
