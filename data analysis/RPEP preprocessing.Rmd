...
setwd( "data")
input <- data.frame()
files <- dir(path = "data", pattern = "RPEP_data__*", full.names = TRUE) #files; double _ so that it detects underscore

for (i in files) {
  tmp <- read.csv (i, header = TRUE) #read the file
  tmp <- tmp[tmp$X<=125, ]## CE: what does this mean?
...
input$startPrev <- lag(input$startRT) # Start RT of trial n-1 # CE: these are leftovers from Frederick's old script, you don't need these.
## and make it numeric
